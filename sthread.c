...
static void _sthread_self_suspend(struct sthread_struct *self)
{
	...
	__attribute__((unused)) ssize_t _unused = read(self->pipe[0], buf, 9);
}
...
static int __attribute__ ((noreturn)) start_thread(void *arg)
{
	...
	__attribute__((unused)) ssize_t _unused = write(manager_thread.pipe[1], &msg, sizeof(manager_msg_t));
...
static int __attribute__ ((noreturn)) manager_main(void *arg)
{
	...
	kill(p->pid, SIGKILL);
		...
		__attribute__((unused)) ssize_t _unused = read(manager_thread.pipe[0], &msg, sizeof(manager_msg_t));
...
{
	__attribute__((unused)) ssize_t _unused = write(t->pipe[1], "wake up!", 9);
}
